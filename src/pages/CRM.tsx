...
const CRM = () => {
  ...
  const handleSync = async () => {
    setSyncing(true)
    try { await apiCRM.sync(); const [c, d] = await Promise.all([apiCRM.contacts(), apiCRM.deals()]); setContacts(c.contacts); setDeals(d.deals) } catch {}
    setSyncing(false)
  }
  ...
  return (
    <PageTransition>
      <div className="space-y-6">
        ...
        <div className="flex gap-1 border-b border-white/[0.06] pb-px">
          {(['contacts', 'deals'] as const).map(t => (
            <button key={t} onClick={() => setTab(t)}
              className={`px-4 py-2 text-sm font-medium rounded-t-lg transition-colors capitalize ${tab === t ? 'text-white bg-white/[0.06] border-b-2 border-blue-500' : 'text-gray-500 hover:text-gray-300'}`}>
        ...
        {loading ? (
          <div className="space-y-2">{[1,2,3,4].map(i => <div key={i} className="h-14 bg-white/5 rounded-xl animate-pulse" />)}</div>
        ) : tab === 'contacts' ? (
          contacts.length === 0 ? (
