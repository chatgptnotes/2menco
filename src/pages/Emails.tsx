...
const Emails = () => {
  ...
  const handleSync = async () => {
    setSyncing(true)
    try {
      await apiEmails.sync()
      const d = await apiEmails.list()
      setEmails(d.emails)
    } catch { /* noop */ }
    setSyncing(false)
  }
  ...
  const handleSummary = async (id: string) => {
    ...
    setSelected(id)
    try {
      const res = await apiEmails.summary(id)
      setSummary(prev => ({ ...prev, [id]: typeof res.summary === 'string' ? res.summary : JSON.stringify(res.summary) }))
    } catch {
      setSummary(prev => ({ ...prev, [id]: 'Could not generate summary' }))
    }
  }
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">Gmail integration with AI summaries</p>
          </div>
        ...
        <div className="grid grid-cols-3 gap-4">
          ...
          <div className="card text-center">
            ...
            <p className="text-2xl font-bold text-white">
              {emails.length > 0 ? new Date(emails[0].received_at).toLocaleDateString() : '—'}
            </p>
            <p className="text-xs text-gray-500">Latest Email</p>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="card text-center py-16">
            ...
            <p className="text-gray-500">No emails yet. Click &quot;Sync Gmail&quot; to fetch emails.</p>
          </div>
        ) : (
          <div className="space-y-2">
            {emails.map((em, i) => (
              ...
              >
                <div className="card-hover cursor-pointer" onClick={() => handleSummary(em.id)}>
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <h3 className="text-sm font-medium text-white truncate">{em.subject || '(no subject)'}</h3>
                        <ChevronRight className={`h-3 w-3 text-gray-600 transition-transform ${selected === em.id ? 'rotate-90' : ''}`} />
                      </div>
                      <p className="text-xs text-gray-500 truncate">{em.from_email}</p>
                    </div>
                    <div className="flex items-center gap-2 flex-shrink-0">
                      <button
                        onClick={e => { e.stopPropagation(); handleSummary(em.id) }}
                  ...
                  {selected === em.id && summary[em.id] && (
                    <motion.div
                      initial={{ height: 0, opacity: 0 }}
                      animate={{ height: 'auto', opacity: 1 }}
                      className="mt-3 pt-3 border-t border-white/[0.06]"
                    >
                      <p className="text-sm text-gray-300">{summary[em.id]}</p>
                    </motion.div>
                  )}
                </div>
              </motion.div>
