/**
 * API client for AgentSDR Flask backend.
...
export const apiAuth = {
  ...
  me: () => apiFetch<{ id: string; email: string; display_name: string; organizations: unknown[] }>('/auth/me'),
}
...
export const apiAgents = {
  ...
  run: (role: string, task?: string, description?: string) =>
    apiFetch(`/agents/${role}/run`, {
      method: 'POST',
      body: JSON.stringify({ task: task || 'default', description }),
    }),
}
...
export const apiEmails = {
  list: (limit = 50, offset = 0) =>
    apiFetch<{ emails: Email[]; total: number }>(`/emails?limit=${limit}&offset=${offset}`),
...
export interface AnalyticsData {
  ...
  deals: { total: number; won: number; pipeline_value: number }
  activity: { id: string; action: string; details: string; created_at: string; agent_type?: string }[]
}
